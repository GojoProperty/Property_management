                ...
                <div class="inner-box mr-0">
                    ...
                    <div class="content-box">
                        ...
                        <ul class="info clearfix mr-0">
                            ...
                            <li><i class="fab fa fa-phone"></i><a href="tel:03030571965">{{ $agent->phone }}</a></li><br>
                        </ul>

                        <div>
                            @php
                                $average = round($agent->averageRating());
                            @endphp
                            ...
                            @if($average)
                                <div>
                                    @for($i = 1; $i <= 5; $i++)
                                        ...
                                        @endif
                                    @endfor
                                    ({{ number_format($agent->averageRating(), 1) }} / 5)
                                </div>
                            @else
                                <p>No ratings yet.</p>
                            @endif
...
<section class="agents-page-section agent-details-page">
    <div class="auto-container">
        <div class="row clearfix">
            ...
            <!-- Agent Rating Form -->
            <div class="agent-rating-box mb-4">
                <div class="widget-title">
                    <h5>Rate {{ $agent->name }}</h5>
                </div>
                <div class="form-inner">
                    <form action="{{ route('agent.rating.submit') }}" method="POST" class="default-form">
                        @csrf
                        ...
                        <div class="form-group">
                            <textarea name="comment" placeholder="Write a comment (optional)" class="form-control"></textarea>
                        </div>
                        
                        <div class="form-group message-btn">
                            <button type="submit" class="theme-btn btn-one">Submit Rating</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="col-lg-4 col-md-12 col-sm-12 sidebar-side">
                <div class="default-sidebar agent-sidebar">
                        ...
                        <div class="form-inner">
                            @auth
                                ...
                                <form action="{{ route('agent.details.message') }}" method="post" class="default-form">
                                    @csrf
                                    <input type="hidden" name="agent_id" value="{{ $agent->id }}">
                                    <div class="form-group"><input type="text" name="msg_name" placeholder="Your name" value="{{ $userData->name }}"></div>
                                    <div class="form-group"><input type="email" name="msg_email" placeholder="Your Email" value="{{ $userData->email }}"></div>
                                    <div class="form-group"><input type="text" name="msg_phone" placeholder="Phone" value="{{ $userData->phone }}"></div>
                                    <div class="form-group"><textarea name="message" placeholder="Message"></textarea></div>
                                    <div class="form-group message-btn"><button type="submit" class="theme-btn btn-one">Send Message</button></div>
                                </form>
                            @else
                                <form action="{{ route('agent.details.message') }}" method="post" class="default-form">
                                    @csrf
                                    <input type="hidden" name="agent_id" value="{{ $agent->id }}">
                                    <div class="form-group"><input type="text" name="msg_name" placeholder="Your name" required=""></div>
                                    <div class="form-group"><input type="email" name="msg_email" placeholder="Your Email" required=""></div>
                                    <div class="form-group"><input type="text" name="msg_phone" placeholder="Phone" required=""></div>
                                    <div class="form-group"><textarea name="message" placeholder="Message"></textarea></div>
                                    <div class="form-group message-btn"><button type="submit" class="theme-btn btn-one">Send Message</button></div>
                                </form>
                            @endauth
                    ...
                    <div class="category-widget sidebar-widget">
                        <div class="widget-title"><h5>Status Of Property</h5></div>
                    ...
                    <div class="featured-widget sidebar-widget">
                        <div class="widget-title"><h5>Featured Properties</h5></div>
                    ...
                    <div>
                        @if($agent->agentRatings->whereNotNull('comment')->count())
                            <div class="agent-comments mt-3">
                                <h5>User Comments:</h5>
                                @foreach($agent->agentRatings->whereNotNull('comment') as $rating)
                                    <div class="comment-box mb-2 p-2" style="background: #f8f8f8; border-radius: 6px;">
                        ...
                        @else
                            <p class="text-muted">No user comments yet.</p>
                        @endif
                    </div>
